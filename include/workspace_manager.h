...
public:
     ...
     **/
    explicit RNNTWorkspaceManager(const dtype *const acts, const int *const labels, const int B, const int *T,
                                  const int *S, const int V)
            : T_(T), S_(S), B_(B), V_(V), workspace_(nullptr), acts_(acts), labels_(labels), alphas_(B), betas_(B),
              denom_(B), dtype_size_(sizeof(dtype)), act_start_indices_(B), S_max_(*std::max_element(S, S + B)) {
    ...
    [[nodiscard]] int T(int b) const {
        return T_[b];
    }

    [[nodiscard]] int S(int b) const {
        return S_[b];
    }

    [[nodiscard]] int alpha_s_min(int b, int t) const {
        return std::max(0, t - (T_[b] - 1 - S_[b]));
    }

    [[nodiscard]] int alpha_s_max(int b, int t) const {
        return std::min(t + 1, S_[b]);
    }

    [[nodiscard]] int beta_s_min(int b, int t) const {
        return std::max(0, t - (T_[b] - S_[b]));
    }

    [[nodiscard]] int beta_s_max(int b, int t) const {
        return std::min(t, S_[b]);
    }
    ...
    [[nodiscard]] int label(int b, int s) const {
        assert(0 <= b);
        assert(b <= B_);
        assert(0 <= s);
        assert(s < S_[b]);
    ...
    [[nodiscard]] dtype act(int b, int t, int s, int v) const {
        return acts_[act_index(b, t, s, v)];
    }
    ...
    void set_alpha(int b, int t, int s, dtype value) {
        alphas_[b][alpha_idx_(b, t, s)] = value;
    }
    ...
    void set_beta(int b, int t, int s, dtype value) {
        betas_[b][beta_idx_(b, t, s)] = value;
    }
...
private:
    ...
    [[nodiscard]] size_t calc_denom_space_(int b) const {
        return dtype_size_ * T_[b] * (S_[b] + 1);
    }
    ...
    [[nodiscard]] size_t calc_fwdbwd_var_space_(int b) const {
        ...
        //  # # # # # . .
        //  # # # # . . .
        // which decomposes to S + 1 rows of length T + 1 - S each minus 1 state in the last row.
        // This is symmetric for the betas
    ...
    [[nodiscard]] int alpha_idx_(int b, int t, int s) const {
        ...
        // sum of columns to the left of current timestep + s - a height offset inside the right triangle
        // e.g. alphas[t = 1, s = 1] is at index 3 in the array
        // or alphas[t = 2, s = 3] is at index 8
        // or alphas[t = 5, s = 2] is at index 16
